...
test('renders decision-critical offer information', () => {
  ...
  expect(screen.getByText('Oui probable')).toBeInTheDocument();
  expect(screen.getByText('Vérifié <24h')).toBeInTheDocument();
  expect(screen.getByText('Officiel / direct')).toBeInTheDocument();
  expect(screen.getByLabelText('Dans la shortlist')).toBeInTheDocument();
});
