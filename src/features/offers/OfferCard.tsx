    ...
    <article className={`offer-card card${selected ? ' offer-card--selected' : ''}`}>
      <button className="offer-card__button" type="button" onClick={onSelect} aria-pressed={selected}>
        <div className="offer-card__topline">
          ...
          <span className="offer-card__star" aria-label={offer.shortlist ? 'Dans la shortlist' : 'Pas dans la shortlist'} aria-hidden={false}>{offer.shortlist ? '★' : '☆'}</span>
        </div>
        ...
        <div className="offer-card__meta">
          ...
          {offer.specialization ? <span>{offer.specialization}</span> : null}
        </div>
        <div className="offer-card__badges">
          ...
          {offer.m2Fit ? <Badge tone={offer.m2Fit.startsWith('Oui') ? 'success' : 'warning'}>{offer.m2Fit}</Badge> : null}
        </div>
        <div className="offer-card__footer">
          <span>{offer.freshness ?? 'Fraîcheur inconnue'}</span>
          <span>{offer.sourceQuality ?? 'Source à vérifier'}</span>
        </div>
      </button>
    </article>
  );
